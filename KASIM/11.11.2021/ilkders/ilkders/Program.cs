...
namespace ilkders
{
    class Program
    {
        ...
        boygirisi:
            ...
            //Programın Sürekli Çalışması Örneği
            /*
                    string cinsiyet;
                    ...
                    while (durum)
                    {
                        ...
                        switch (cinsiyet)
                        {
                            ...
                            case "erkek":
                                ...
                                else { Console.WriteLine("Aşırı Kilolu" + index); }
                                break;

                            case "kadın":
                                say++;
                                index = ((0.9 * kilo) / (boy * boy)) * 10000;
                                if (index < 18.5)
                                {
                        ...
                        Console.WriteLine(say + "kişinin boy/kilo endexi hesaplandı");
                        Console.ReadKey();
                    }
                kapat:
                    durum = !durum;
                    while (true)
                    {
                        Console.WriteLine("Boy kilo endexini aç");
                        bool durum2 = Convert.ToBoolean(Console.ReadLine());
                        durum = durum2;
                        if (durum == true)
                        {
                            goto basla;
            ...
            */
            //Programın Sürekli Çalışması Örneği Sonu


            //For döngüsünün true ile kullanımı
                    
                    for(; true;)
                    { 
                       //true olduğu sürece sonsuz olarak bu for döngüsü çalışacaktır

                    }
            //For döngüsünün true ile kullanımı sonu

            //Foreach ve do-while kullanımı
            string[] isimler1 = { "Ahmet", "Mehmet", "Ali", "Veli" };
            string ad = "ahmet";

            do
            {
                Console.WriteLine("Sen Ahmetmisin ?"); // while dan önce bu kod bloğu bir kez çalışacak ve daha sonra while içinde yazan koşulu kontrol edecek.
            }
            while (ad == "ahmet");

          
            Console.WriteLine(isimler1[0]);//direkt yazılır
            Console.WriteLine("-------------------------------");
            for (int i5 = 0; i5 < isimler1.Length; i5++)
            {
                Console.WriteLine(isimler1[i5]); //Tüm isimler for döngüsü ile sırası ile yazılacak
            }
            Console.WriteLine("-------------------------------");
            int i8 = 0;

            while (i8 < isimler1.Length)
            {
                Console.WriteLine(isimler1[i8]);
            ...
            while (true)
            {
                Console.WriteLine(isimler1[i9]);

                if (i9 == isimler1.Length)
                {
                    break;
                }
            }
            ...
            string[] isimler3 = { "Ahmet", "Mehmet", "Ali", "Veli" };

            foreach (string dizielemani in isimler3) //foreach yazılırken kullanılan dizi tipinde tanımlanmalıdır. String için bu şekilde kullanım saglanır.
            {
                Console.WriteLine(dizielemani); 
            }

            int[] sayilardizisi1 = { 5, 33, 4, 2 };

            foreach(int sayielemani in sayilardizisi1)
            { //foreach yazılırken kullanılan dizi tipinde tanımlanmalıdır. Int için bu şekilde kullanım saglanır.

                Console.WriteLine(sayielemani);
            }
            //Foreach ve do-while kullanımı sonu
            //Döngü Kullanımları Sonu

            //Metod Oluşturma

                    /*Farklı Metod Çeşitleri                  Metodlar İki Başlıkta İncelenir  
                     * Değer Almayan Değer Döndürmeyen      *Public:Public metod tüm classlarda kullanılabilir çağrılabilir değer alıp gönderilebilir.
                     * Değer Alan Değer Döndürmeyen         *Private:Private sadece oluşturulduğu class içinde kullanılır ve dışarıdan çağrılamaz.
                     * Değer Almayan Değer Döndüren
                     * Değer Alan Değer Döndüren
                     */

          
            //Metod Oluşturma Sonu

        }

        public void metod()
        {
            Console.WriteLine("Merhaba Metod Fonksiyonu");
        }
